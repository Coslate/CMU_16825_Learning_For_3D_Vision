...
class ImageConditionedOccupancyDecoder(nn.Module):
    def __init__(self, feature_dim=512, hidden_dim=768):
        super().__init__()
        self.fc_in = nn.Linear(feature_dim + 3, hidden_dim)

        self.resblocks = nn.Sequential(
            *[ResNetBlock(hidden_dim) for _ in range(8)]  # Increase from 5 to 8
        )

        self.fc_out = nn.Linear(hidden_dim, 1)  # Logits output

    def forward(self, z, coords):
        x = torch.cat([z.repeat(coords.shape[0] // z.shape[0], 1), coords], dim=-1)
